...
module.exports = function (objectConf) {
  ...
  function isAuthenticatedFunction(req, res) {
    return new Promise(function (resolve, reject) {
      ...
      if (authObj.auth) {
        if (objectConfiguration.authType === 'db') {
        ...
        } else if (objectConfiguration.authType === 'jsToken') {
          var token = req.body.token || req.query.token || req.headers['x-access-token'];
          if (token) {
            verifyToken(token)
              .then(function (decoded) {
                req.diyAuth.tokenDecoded  = decoded;
                req.diyAuth.authenticated = true;
                req.diyAuth.hasPermission = authObj.accessByRole.indexOf(decoded.role) !== -1;
  ...
  /*  function isAuthenticatedFunctionToken(req, res) {
      return new Promise(function (resolve, reject) {
        createRequestAuthObject(req);
  ...
  /*  function loginFunctionToken(req, res, user, pass) {
      // 1. login, el usuario se loguea
      // 2. se busca en la base de datos el usuario y contraseña
      //    2.1.  ocurre un error
      //    2.2.  no encuentra el usuario
  ...
  function createToken(data) {
    return Promise.resolve(function (data) {
      return jwt.sign(data, objectConfiguration.password, {
        expiresInMinutes: objectConfiguration.expiredTokenTime / 1000
      });
    });
  }
  ...
  /*  function logoutFunctionToken(req, res) {
      // esta funcion no tiene ni que existir, con que borre el token localmente
      // ya queda deslogueado
    }*/
  ...
  var objectConfiguration = {
    ...
    expiredTokenTime        : 1000 * 60 * 60 * 24 * 365,
    token                   : '',
    authType                : '',  // db - jsToken
    /*
     routes
  ...
  function createRequestAuthObject(req) {
    req.diyAuth = {
      ...
      token          : objectConfiguration.token,
      tokenDecoded   : ''
    };
  }
  ...
  function verifyToken(token) {
    return new Promise(function (resolve, reject) {
      jwt.verify(token, objectConfiguration.password, function (err, decoded) {
        if (err) reject(err);
...
/*  function isAuthenticatedMiddelwareToken(req, res, next) {
    createRequestAuthObject(req);
